...
local_geary_plot = \(sff, col_name, w_t, per_n, cpu_n, cut_off_n) {
  if (w_t == "queen") {
  ...
  lisa_sf_df = sff |>
    ...
    dplyr::mutate(
      lisaGeary = rgeoda::lisa_values(lisa),
...
local_geary_plot(raw_data, "Crm_prp", "queen", 999, 8, 0.05) -> geary_df
...
geary_df |>
  ggplot() +
  geom_sf(aes(fill = lisaLabIndex)) -> p_g
